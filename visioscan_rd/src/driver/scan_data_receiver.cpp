...
ScanDataReceiver::ScanDataReceiver(const std::string topic, const std::string frame, const std::string hostname, const int port, BEA_PARAMETER_INFO bpi) : inbuf_(4096), instream_(&inbuf_), ring_buffer_(65536), scan_data_()
{
    ...
    {
        rosScanInfo_.ros_angle_min       = (parameterInfo_.stopAngle - 90.0) * TO_RADIAN;
        rosScanInfo_.ros_angle_max       = (parameterInfo_.startAngle - 90.0) * TO_RADIAN;
        rosScanInfo_.ros_angle_increment = -parameterInfo_.angularResolution * (parameterInfo_.skipSpots + 1) * TO_RADIAN;
    }
    else // Counterclockwise
    {
        rosScanInfo_.ros_angle_min       = (parameterInfo_.startAngle - 90.0) * TO_RADIAN;
        rosScanInfo_.ros_angle_max       = (parameterInfo_.stopAngle - 90.0) * TO_RADIAN;
        rosScanInfo_.ros_angle_increment = parameterInfo_.angularResolution * (parameterInfo_.skipSpots + 1) * TO_RADIAN;
    }
...
bool ScanDataReceiver::handleNextPacket()
{
    ...
    if(p->header.subNum == p->header.totalNum)
    {
        ...
        if(p->header.packetType == 1)
        {
            ...
            {
                scanmsg.ranges[i]      = float(distanceData_[i]) / 1000.0f;
                scanmsg.intensities[i] = float(intensitiesData_[i]);
...
bool ScanDataReceiver::sendMDI(void)
{
    bool rtcd = true;
    boost::system::error_code ec;

    tcp_socket_->write_some(boost::asio::buffer(CMD_cWN_SendMDI, 13), ec);
    if(ec)
    {
...
bool ScanDataReceiver::stopMDI(void)
{
    bool rtcd = true;
    boost::system::error_code ec;

    tcp_socket_->write_some(boost::asio::buffer(CMD_cWN_StopMDI, 13), ec);
    if(ec)
    {
